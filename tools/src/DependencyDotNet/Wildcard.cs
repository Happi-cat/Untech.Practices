...
namespace DependencyDotNet
{
	...
	{
		public static bool IsMatchAnyMask(string input, IEnumerable<string> masks)
		{
			return masks != null && masks.Any(mask => IsMatch(input, mask));
		}
		...
		public static bool IsMatch(ReadOnlySpan<char> input, ReadOnlySpan<char> mask)
		{
			...
			for (int i = 0; i < mask.Length; i++)
			{
				...
				if (maskChar == '?')
				{
					if (input.Length == 0)
						return false;
					input = input.Slice(1); // ignore current char
				}
				else if (maskChar == '*')
				{
